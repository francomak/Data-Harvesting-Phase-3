...
def get_parser():
    ...
    parser.add_argument(
        "--bpe-model",
        type=str,
        default=f"{config['datamodule']['lang_dir']}/bpe.model",
        help="Path to the BPE model",
    )

    parser.add_argument(
        "--base-lr", type=float, default=config['parameters']['initial_lr'], help="The base learning rate."
    )
...
def run(rank, world_size, args):
    ...
    csir_dataset = AsrDataModule(args)

    train_cuts = csir_dataset.train_cuts()
    ...
    train_dl = csir_dataset.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict
    )

    valid_cuts = csir_dataset.dev_cuts()
    valid_dl = csir_dataset.valid_dataloaders(valid_cuts)
