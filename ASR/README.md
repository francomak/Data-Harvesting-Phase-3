...
- Reuse existing pipelines for new languages/datasets.
- Inspect and compare ASR system outputs.

> **Status:** First public upload. Expect paths and configs to be somewhat project‑specific. Over time this repo will be cleaned up and documented more thoroughly.
...
- `create_train_dev_test_lists.py`  
  ...
  - Uses the available transcriptions and audio durations to:
    ...
    - Randomly shuffle and split according to duration targets (e.g. 30 min dev, 30 min test, rest train).

**If you are preparing a *new dataset***, start by reading `models/preparation_scripts/Readme.txt` and then adapt these three scripts (mainly paths and any dataset‑specific naming conventions).
...
- `Lhotse_data_preparation.py`, `Lhotse_data_preparation_mixed_datasets.py`  
  - Turn your train/dev/test lists into:
...
- `decode.py`, `decode_no_sil.py`, `decode_phone.py`  
...
- `decode.py`, `ctc_decode.py`, `streaming_decode.py`, `onnx_decode.py`, `decode_stream.py`, etc.  
...
1. **Start from raw audio + transcripts.**
   ...
   - Transcription spreadsheet (e.g. from iTranscribe) with timing, flags, filenames, text.
...
- If you’re unsure, ask the maintainer for the environment used in recent experiments.
...
- **Keep dataset‑specific logic clearly marked.**  
  When you hard‑code paths or dataset conventions, add a short comment so others can adapt it.

- **Prefer configs over hard‑coding.**  
  When possible, expose settings via `main_config.yaml` / `configs/` rather than editing core code.

- **Document new scripts.**  
  At minimum, add:
  - A short docstring at the top explaining what the script does.
  - A short usage example (command line & expected inputs/outputs).

- **Non‑specialists:**  
  If something is unclear, open an issue or add a comment near the relevant code describing what you *think* it does; that feedback is valuable for improving this repo.

---

## Contact

For questions about how to run specific experiments or how to adapt these tools to a new dataset, please contact the maintainer of this repo.
