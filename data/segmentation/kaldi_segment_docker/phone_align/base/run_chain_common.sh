...
if [ $stage -le 11 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt data/lang_${lang_exp}/L.fst ]; then
    ...
    echo data/lang_${lang_exp}
    cp -r data/lang_${lang_exp} $lang
...
#     $lang $gmm_dir $lat_dir
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" --retry-beam 160 ${lores_train_data_dir} \
    $lang $gmm_dir $lat_dir
