...
def createWavScp(list_file, audio_dir, out_dir):
    ...
    sub_dir = "Clean_transcriptions" #Just for sabn data
    ...
    for fname in fnames:
        disk_dir = fname.split("_")[1]
        out_line = " ".join([fname, "sox", os.path.join(audio_dir, disk_dir, sub_dir, ".".join([fname,"wav"])) ,"-t","wav","-r","16000","-","|"])
        file_handle.write("".join([out_line,"\n"]))
    file_handle.close()
...
def prepareData(audio_dir, trans_dir, lang, list_dir):
    ...
    train_list = os.path.join(list_dir, "train.lst")
    createTxtScp(train_list, trans_dir, train_dir)
    createWavScp(train_list, audio_dir, train_dir)
    ...
    os.makedirs(dev_dir)
    dev_list = os.path.join(list_dir, "dev.lst")
    createTxtScp(dev_list, trans_dir, dev_dir)
    createWavScp(dev_list, audio_dir, dev_dir)
    createReco(dev_dir)
