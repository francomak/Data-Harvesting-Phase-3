...
def calculateScores(tra_dir, data_dir, lang_or_graph_dir, decode_dir):
    ...
    for tra_file in tra_files:
        if ".tra.txt.sil" in tra_file and os.path.splitext(tra_file)[1] == ".sil":
            ...
            cmd_string = " ".join(["compute-wer --text --mode=present", "".join(["ark:",os.path.join(tra_dir, "test_filt.txt"),".sil"]), "".join(["ark:",os.path.join(tra_dir, tra_file)]),"> ", os.path.join(decode_dir, "".join(["sper_",tra_file.split(".tra.txt.sil")[0]]))])
            print cmd_string
