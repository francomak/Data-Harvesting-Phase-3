...
def createTxtScp(list_file, trans_dir, out_dir):
    ...
    for fname in fnames:
        spk_dir = fname.split("/")[0]
        fname_short = fname.split("/")[1]
        out_line = " ".join([fname_short, os.path.join(trans_dir,".".join([fname_short,"txt"]))])
...
def createWavScp(list_file, audio_dir, out_dir):
    ...
    for fname in fnames:
        spk_dir = fname.split("/")[0]
        fname_short = fname.split("/")[1]
        out_line = " ".join([fname_short, "sox", os.path.join(audio_dir, spk_dir,".".join([fname_short,"wav"])) ,"-t","wav","-r","16000","-","|"])
        file_handle.write("".join([out_line,"\n"]))
    file_handle.close()
...
def createSpkMaps(out_dir):
    ...
    for line in file_handle:
        fname_tag = line.split()[0]
        spk_tag = "_".join(line.split("_")[0:3])
